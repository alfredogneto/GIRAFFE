...
Node::Node(int e_nGL)
{
	nGL = e_nGL;						//Seta o numero de graus de liberdade por n�
	ref_coordinates = new double[nGL];	//Coordenadas do n� na configura��o de refer�ncia
	copy_coordinates = new double[nGL];	//Coordenadas do n� na configura��o de c�pia (ultima convergida)
	copy_rot_euler = new double[3];
	displacements = new double[nGL];	//Deslocamentos do n� em rela��o a ultima c�pia de coordenadas
	vel = new double[nGL];				//Velocidades
	copy_vel = new double[nGL];		
	accel = new double[nGL];			//Acelera��es
	...
	(*I3)(1, 1) = 1.0;
	(*I3)(2, 2) = 1.0;
	//Inicializa��o da matriz Q
	*Q = *I3;
	//Inicializa��o da matriz Q0
	*Q0 = *I3;
...
void Node::WriteMonitor(FILE *f, bool first_record, double time)
{
	...
	{
		//Convers�o do pseudo-momento para momento (parametros de rota��o de Rodrigues)
		//Calculando o operador Xi
		*A = skew(*rot_rodrigues);			//Matriz A
		g = 4.0 / (4.0 + norm(*rot_rodrigues)*norm(*rot_rodrigues));		//fun��o g(alpha) - em algumas ref. tb. chamado de h(alpha)
		*Xi = g * (*I + 0.5*(*A));
	...
	fprintf(f, "%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\t%.12e\n",
		time,
		//Deslocamentos - global, rota��es pode ser local ou global
		copy_coordinates[0],
		copy_coordinates[1],
		copy_coordinates[2],
		copy_rot_euler[0],
...
{
	// Montagem dos esfor�os (id�ntica ao WriteMonitor)
	(*rot_rodrigues)(0, 0) = displacements[3];
	(*rot_rodrigues)(1, 0) = displacements[4];
	(*rot_rodrigues)(2, 0) = displacements[5];
	...
	// Interpola��o dos esfor�os via tubos conectados (igual ao WriteMonitor)
	std::vector<std::vector<double>> vector_loads(6);
	for (Pipe_1* pipe : attached_pipes)
	{
		if (!pipe || !pipe->sigma_r[0] || !pipe->sigma_r[1]) continue;
...
void Node::PreCalc()
{
	for (int i = 0; i < db.number_elements; i++) {
		//Verifica se o n� pertence a um elemento do tipo Pipe_1
		Pipe_1* pipe = dynamic_cast<Pipe_1*>(db.elements[i]);
		if (pipe != nullptr)
		{
			//Verifica se o n� pertence ao elemento
...
{
	//Atualiza��o de deslocamentos (descri��o lagrangiana atualizada - incremental)
	copy_coordinates[0] += displacements[0];
	copy_coordinates[1] += displacements[1];
	copy_coordinates[2] += displacements[2];

	//Atualiza��o de rota��es (descri��o lagrangiana atualizada - incremental)
	...
	if (flag_material_description == false)
		alpha_3 = 4.0 / (4.0 - dot(alpha_2, alpha_1))*(alpha_2 + alpha_1 + 0.5*cross(alpha_2, alpha_1));	//Vetor rota��o de Rodrigues - atualiza��o de descri��o espacial
	else
		alpha_3 = 4.0 / (4.0 - dot(alpha_1, alpha_2))*(alpha_1 + alpha_2 + 0.5*cross(alpha_1, alpha_2));	//Vetor rota��o de Rodrigues - atualiza��o de descri��o material
	...
	*Q = *I + g * (*A + 0.5*(*A)*(*A));
	...
	//Atualiza��o de outros graus de liberdade (descri��o lagrangiana total)
	for (int j = 6; j < db.number_GLs_node; j++)
	{
		copy_coordinates[j] = displacements[j];
	}

	//Atualiza��o das velocidades e acelera��es
	for (int j = 0; j < db.number_GLs_node; j++)
	{
