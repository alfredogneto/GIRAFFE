...
Pipe_1::Pipe_1()
{
	...
	g = 0;
	//Cada variavel carregara a informa��o para seu ponto de Gauss
	N1 = new double[2];
	N2 = new double[2];
	N3 = new double[2];
	dN1 = new double[2];
	...
	//Variaveis internas para o esfor�o de correnteza maritima
	e3ip = new Matrix*[2];
	zi = new Matrix*[2];
	vel = new Matrix*[2];
	velr = new Matrix*[2];
	...
	Aext = 0;
	rho_f = 0;
	//Variaveis internas para o esfor�o pipe load
	kip = new Matrix*[2];
	temp_f = new Matrix*[2];
	temp_m = new Matrix*[2];
	temp_l = new Matrix*[2];
	...
	//Ponteiros double** - convers�o de matriz
	pJr = new double*[3];
	for (int i = 0; i < 3; i++)
		pJr[i] = new double[3];
	pMr = new double*[3];
...
bool Pipe_1::Check()
{
	...
	if (section > db.number_pipe_sections)
		return false;
	//Checagem do alinhamento dos eixos do CS com dire��o dos n�s
	Matrix e1r = *db.CS[cs - 1]->E1;
	Matrix e2r = *db.CS[cs - 1]->E2;
	Matrix e3r = *db.CS[cs - 1]->E3;
	double tolortho = 1e-8;
	Matrix e3_check(3);
	//Versor e3_check (configura��o de refer�ncia)
...
{
	//vetores para escrita no formato binario - usando a fun��o 'enconde'
	...
	fprintf(f, "\t\t<Piece NumberOfPoints = \"%d\" NumberOfCells = \"%d\">\n", n_points, n_cells);
		...
		fprintf(f, "\t\t\t<Points>\n");
			...
			for (int i = 0; i < n_nodes; i++)//percorrendo os 3 n�s do elemento
			{
				//Para cada n� do elemento, calcula o tensor rota��o
				...
				Q = *I3 + g*(A + 0.5*A*A);
				Q = Q*transp(*transform3);//Matriz de transforma��o para trazer o vetor da ST do plano xy para o plano atual em que ela se encontra

				for (int point = 0; point < db.pipe_sections[section - 1]->sec_details->n_points; point++)//Percorre os n�s que descrevem o perimetro da ST - diametro externo
				{
					//Posi��o de cada ponto P no plano xy (refer�ncia)
					...
					for (int c = 0; c < 3; c++)
						vec_P(c, 0) += db.nodes[nodes[i] - 1]->ref_coordinates[c] + db.post_files->mag_factor*(db.nodes[nodes[i] - 1]->copy_coordinates[c] - db.nodes[nodes[i] - 1]->ref_coordinates[c]);//Transla��o - soma a posi��o do centro da barra (do ponto em quest�o)
					float_vector.push_back((float)(vec_P(0, 0)));
					float_vector.push_back((float)(vec_P(1, 0)));
					float_vector.push_back((float)(vec_P(2, 0)));
				}
				for (int point = 0; point < db.pipe_sections[section - 1]->sec_details->n_points; point++)//Percorre os n�s que descrevem o perimetro da ST - diametro interno
				{
					//Posi��o de cada ponto P no plano xy (refer�ncia)
		...
		{
			//Se n�o for a ultima celula ao longo do perimetro
...
// --- Helpers gen�ricos de desmembramento para WriteMonitorUserDef ---
...
// Tipo 4: Matrix* (matriz �nica de qualquer dimens�o)
...
// Tipo 5: std::vector<double> (tamanho vari�vel)
...
void Pipe_1::WriteMonitorUserDef(FILE* f, bool first_record, double time, const UserDefElemMonitorParams& params)
{
	...
	// Cabe�alho (escrito apenas na primeira grava��o deste arquivo)
	if (first_record)
	{
		fprintf(f, "TIME\t");
...
void Pipe_1::Mount()
{
	...
	for (int gauss = 0; gauss < 2; gauss++)
	{
		...
		//Transforma��o para o sistema local do elemento
		...
		*d_u_delta[gauss] = (*transform3)*(*d_u_delta[gauss]);
		//Calculo de tensores de rota��o e outros
		...
		*Q_delta[gauss] = *I3 + g*(*A_delta[gauss] + 0.5*((*A_delta[gauss])*(*A_delta[gauss])));							//Tensor de rota��o
		...
		*kappa_r[gauss] = transp(*lag_save->Q_i[gauss])*transp(*Xi_delta[gauss])*(*d_alpha_delta[gauss]) + (*lag_save->kappa_i_ref[gauss]);
		//Vetor deforma��o generalizada retro-rotacionada
		...
		//For�as e momentos internos retro-rotacionados
		...
		//For�as e momentos internos n�o retro-rotacionados
		...
		*G_alpha_d_alpha_transp = *V_alpha_m;
		...
		//Esfor�os internos calculados
		(*i_loading) = (*i_loading) + (alpha1*jacobian)*(((transp(*deltaN[gauss])) * (transp(*B))) * (*sigma_r[gauss]));
	}//end of gauss loop
	//Transforma��es de coordenadas, para o sistema global - matriz de rigidez
...
{
	MountFieldLoading();		//Inclui atualiza��es no vetor e_loading
	MountSeaCurrentLoading();	//Inclui atualiza��es no vetor e_loading e matriz loading_stiffness e stiffness
	(*P_loading) = (*i_loading) - (*e_loading);		//Vetor esfor�o desbalanceado
}
...
void Pipe_1::TransformMatrix()
{
	...
	Matrix e2r = *db.CS[cs - 1]->E2;
	Matrix e3r = *db.CS[cs - 1]->E3;
	//Preenche a matriz de transforma��o de coordenadas
...
//Salva variaveis nos pontos de Gauss uteis para descri��o lagrangiana atualizada
void Pipe_1::SaveLagrange()
{
	//Loop nos pontos de Gauss
	for (int gauss = 0; gauss < 2; gauss++)
	{
		(*lag_save->Q_i[gauss]) = (*Q_delta[gauss])*(*lag_save->Q_i[gauss]);										//Tensor de rota��o
		(*lag_save->u_i[gauss]) = (*lag_save->u_i[gauss]) + *u_delta[gauss];										//Deslocamento
		(*lag_save->kappa_i_ref[gauss]) = *kappa_r[gauss];															//Rota��o por unidade de comprimento 
		(*lag_save->alpha_i[gauss]) = 4.0 / (4.0 - dot(*alpha_delta[gauss], *lag_save->alpha_i[gauss]))* (*alpha_delta[gauss] +
			(*lag_save->alpha_i[gauss]) + 0.5*cross(*alpha_delta[gauss], *lag_save->alpha_i[gauss]));				//Vetor rota��o de Rodrigues
...
void Pipe_1::PreCalc()
{
	...
	(*B2)(1, 1) = 1.0;
	(*B2)(2, 2) = 1.0;
	//Monta as matrizes de rota��o
	TransformMatrix();
	//Versor e3r (configura��o de refer�ncia) - escrito no sistema local do elemento
	...
	*e3r = (1.0 / (norm(*e3r)))*(*e3r);							//Normaliza��o
	*e3rg = *e3r;												//C�pia do e3r (preservado no sistema global)
	*e3r = (*transform3)*(*e3r);								//e3r escrito no sistema local do elemento
	//Massa por unidade de comprimento na configura��o de refer�ncia
	...
	//Inercia por unidade de comprimento na configura��o de refer�ncia
	...
	//Termos de transporte - alterar em situa��es em que o baricentro n�o e o polo utilizado no calculo do momento de inercia
	(*br)(0, 0) = 0.0;
	(*br)(1, 0) = 0.0;
	(*br)(2, 0) = 0.0;
...
//Monta a parte estatica dos esfor�os de Morison - correnteza maritima
	...
	if (db.environment_exist == true)
	{
		if (db.environment->ocean_data_exist == true)
		{
			...
			if (mult != 0.0)
			{
				...
				(*e_loading) = (*e_loading) + transp(*transform)*(*morison_loading);				//For�as de Morison
			}
		}
	}
}

//Monta carregamentos de press�o interna/externa no tubo
void Pipe_1::MountPipeSpecialLoads(int l_number)
{
	//Fun��o para montar carregamentos de press�o em tubos. Atende simula��es estatica e dinamica
	//////////////////Efeito da Press�o////////////////////////////
	...
	for (int gauss = 0; gauss < 2; gauss++)
	{
		*kip[gauss] = ((*Q_delta[gauss])* (*lag_save->Q_i[gauss]))*(*kappa_r[gauss]);	//Rota��o especifica no ponto de gauss no instante i+1 (Lag. Atualizada)
		*e3ip[gauss] = ((*Q_delta[gauss])* (*lag_save->Q_i[gauss]))*(*e3r);				//Orienta��o do elemento no ponto de gauss no instante i+1 (Lag. Atualizada)
		//For�as - sistema local (elemento)
		...
		*temp_m[gauss] = -p0i*Aint*transp(*Xi_delta[gauss])*cross(*d_z[gauss], *e3ip[gauss]);
		//Vetor de esfor�os externos generalizados - sistema local (elemento)
		...
		*K2au = transp(*Xi_delta[gauss])*(*E3ip[gauss]);
		...
		(*P_loading) = (*P_loading) - mult* transp(*transform)*(transp(*N[gauss]))*(*temp_l[gauss]);	//Esfor�o desbalanceado
		(*e_loading) = (*e_loading) + mult* transp(*transform)*(transp(*N[gauss]))*(*temp_l[gauss]);	//Esfor�o externo
		//Contribui��o para a matriz de rigidez
...
void Pipe_1::MountMassModal()
{
	...
	for (int gauss = 0; gauss < 2; gauss++)
	{
		...
		{
			//Avalia se o tubo esta imerso ou n�o para modificar a massa adicional
			//Posi��o do ponto de Gauss
			(*zi[gauss])(0, 0) = (db.nodes[nodes[0] - 1]->copy_coordinates[0])* N1[gauss] +
				(db.nodes[nodes[1] - 1]->copy_coordinates[0])* N2[gauss] +
				(db.nodes[nodes[2] - 1]->copy_coordinates[0])* N3[gauss] +
	...
	//Convers�o para o sistema global
	(*mass_modal) = (transp(*transform)*(*mass_modal))*(*transform);
}

//Monta a matriz de amortecimento para realiza��o da analise modal
...
void Pipe_1::MountDamping(bool update_rayleigh)
{
	if (typeid(*db.solution[db.current_solution_number - 1]) == typeid(Dynamic))
	{
		...
		(*damping) = (*damping) + ptr_sol->a4*(*rayleigh_damping);	//Atualizando a matriz de amortecimento - inclus�o do efeito de Rayleigh

		Matrix v_ipp(18);
		//Calculo dos esfor�os de amortecimento - utiliza��o de informa��o das velocidades nos GL do elemento
...
//Composi��o da forma fraca e operador tangente - contribui��es inerciais e amortecimento
...
//Montagens para analise modal - inser��o da matriz de massa e amortecimento na matriz de rigidez para posterior montagem global
