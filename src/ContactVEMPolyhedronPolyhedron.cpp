...
ContactVEMPolyhedronPolyhedron::ContactVEMPolyhedronPolyhedron()
{
	...
	cur_active = false;
	invert = false;
	currently_inverted_index = false;
...
void ContactVEMPolyhedronPolyhedron::PreCalc()
{
	//Copying and inverting indexes
	int copy_index1 = index1;
	int copy_index2 = index2;
	int copy_sub_index1 = sub_index1;
	int copy_sub_index2 = sub_index2;
	if (invert == true && currently_inverted_index == false)
	{
		//Invert indexes
		index1 = copy_index2;
		index2 = copy_index1;
		sub_index1 = copy_sub_index2;
		sub_index2 = copy_sub_index1;
		currently_inverted_index = true;
	}
	...
	//Reverting inversion
	if (invert == true && currently_inverted_index == true)
	{
		index1 = copy_index1;
		index2 = copy_index2;
		sub_index1 = copy_sub_index1;
		sub_index2 = copy_sub_index2;
		currently_inverted_index = false;
	}
}

void ContactVEMPolyhedronPolyhedron::InsertNewContact(int deg_pointA, int deg_curveA, int deg_pointB, int deg_curveB, int faceAID, int faceBID)
{
	//Copying and inverting indexes
	int copy_index1 = index1;
	int copy_index2 = index2;
...
int ContactVEMPolyhedronPolyhedron::CreateSurfacePair(int deg_pointA, int deg_curveA, int deg_pointB, int deg_curveB, int faceAID, int faceBID)
{
	int list_index = -1;
	for (int i = 0; i < contact_pairs.size(); i++)
	{
...
void ContactVEMPolyhedronPolyhedron::ProcessSurfacePair(int list_index)
{
	contact_pairs[list_index]->eligible = false;
	contact_pairs[list_index]->SolveLCP();
	contact_pairs[list_index]->EvaluateNormalGap();
}
void ContactVEMPolyhedronPolyhedron::ProcessSurfacePairs()
{
	for (int i = 0; i < (int)contact_pairs.size(); i++)
		ProcessSurfacePair(i);
}

void ContactVEMPolyhedronPolyhedron::FinalProcessSurfacePairExplicit(int list_index, double t)
{
	//Copies
...
void ContactVEMPolyhedronPolyhedron::FinalProcessSurfacePairsExplicit(double t)
{
	for (int i = 0; i < (int)contact_pairs.size(); i++)
		FinalProcessSurfacePairExplicit(i,t);
}

void ContactVEMPolyhedronPolyhedron::MountGlobalExplicit()
{
	//TODO-Explicit
}

void ContactVEMPolyhedronPolyhedron::MountGlobal()
{
	//Copying and inverting indexes
	int copy_index1 = index1;
	int copy_index2 = index2;
		...
		if (contact_pairs[c]->GetActive())
		{
			if (contact_pairs[c]->eligible)
			{
				for (int i = 0; i < 15; i++)
				{
					...
					if (i >= 9 && i < 12)
						GL_global_1 = db.nodes[db.particles[index2]->node - 1]->GLs[i - 9];
					if (i >= 12 && i < 15)
						GL_global_1 = db.nodes[db.particles[index2]->node - 1]->GLs[i - 12];
					...
					for (int j = 0; j < 15; j++)
					{
						...
						if (j >= 9 && j < 12)
							GL_global_2 = db.nodes[db.particles[index2]->node - 1]->GLs[j - 9];
						if (j >= 12 && j < 15)
							GL_global_2 = db.nodes[db.particles[index2]->node - 1]->GLs[j - 12];
...
void ContactVEMPolyhedronPolyhedron::ProcessContactHierarchy()
{
	//Copying and inverting indexes
	int copy_index1 = index1;
	int copy_index2 = index2;
					...
					for (int subcont = 0; subcont < db.gcs->contactPB_list[index1].size() && checked == false; subcont++)
					{
								...
								db.gcs->contactPP_list[index1][subcont]->sub_index2 != sub_index2))
						{
							for (int subc = 0; subc < (int)db.gcs->contactPP_list[index1][subcont]->contact_pairs.size() && checked == false; subc++)
							{
								...
								{
									ContactVEMPolyhedronPolyhedron* subptr = static_cast<ContactVEMPolyhedronPolyhedron*>(db.gcs->contactPP_list[index1][subcont]);
