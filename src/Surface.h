...
public:
	...
	bool pilot_is_used;										//flag que indica se o pilot node é usado
	...
	virtual void Gamma_and_Triad(Matrix* G_p, Matrix* t1_p, Matrix* t2_p, Matrix* n_p, Matrix* G_i, Matrix* t1_i, Matrix* t2_i, Matrix* n_i, Matrix* G_ip, double* zi, double* thi, double* zp, double* thp) = 0;				//Calcula diversos vetores associados à superfície para um par de coordenadas convectivas
	virtual void FindMinimimumParameters(Matrix* xS, NSContactData* cd) = 0;			//Dado o ponto xS, calcula as coordenadas (zeta,theta) referentes à mínima distância
	virtual void FillNodes() = 0;														//Atualiza as variáveis internas da superfície, para pegarem info do pilot node para uso posterior com posição atualizada
	virtual void ContactSphereSurfaceSticking(double* Rc, double** Kc, double zetap, double thetap, double zetai, double thetai, double* gti, int node, double* epsn, double* epst, double* cn, double* ct, double* mu, double* radius) = 0;		//Calcula contribuições de contato entre esfera e superfície
	virtual void ContactSphereSurfaceSliding (double* Rc, double** Kc, double zetap, double thetap, double zetai, double thetai, double* gti, int node, double* epsn, double* epst, double* cn, double* ct, double* mu, double* radius) = 0;		//Calcula contribuições de contato entre esfera e superfície
	virtual void SaveConfiguration() = 0;												//Salva vetores de configuração convergida
	virtual void CenterPoint(Matrix* center) = 0;										//Retorna coordenadas globais do ponto central da superfície a ser utilizado para cálculos grosseiros de sua localização (pinball)
	virtual void InitialGuess(Matrix* xS, double** convective, int n_solutions) = 0;	//Realiza chute inicial para as variáveis zeta e theta
