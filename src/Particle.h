...
public:
	Particle();
	virtual ~Particle();
	int number;				//ID da partícula
	int material;			//ID do material
	int cs;					//ID do sistema de coordenadas da partícula
	char* type_name;		//Nome do tipo de partícula
	...
	void WriteMonitor(FILE *f, bool first_record, double time);	//Escreve no monitor da partícula
