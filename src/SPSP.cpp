						...
						if (cd[surf1_index][surf2_index]->return_value[sol_index] == 0 && cd[surf1_index][surf2_index]->g_n[sol_index] < -pen_tol)
						{
							...
							for (int i = 0; i < (DOFs_surfaces1[surf1_index] + DOFs_surfaces2[surf2_index]); i++)
							{
								if (i < DOFs_surfaces1[surf1_index]) {
									...
									GL_global_1 = *db.splines[n_SP1 - 1]->sp_element[tempsurf1]->GLs[i];				//GLs da superfície 1
								}
								else
									GL_global_1 = *db.splines[n_SP2 - 1]->sp_element[tempsurf2]->GLs[i - DOFs_surfaces1[surf1_index]];		//GLs da superfície 2
								...
								for (int j = 0; j < (DOFs_surfaces1[surf1_index] + DOFs_surfaces2[surf2_index]); j++)
								{
									if (j < DOFs_surfaces1[surf1_index])
										GL_global_2 = *db.splines[n_SP1 - 1]->sp_element[tempsurf1]->GLs[j];									//GLs da superfície 1
									else
										GL_global_2 = *db.splines[n_SP2 - 1]->sp_element[tempsurf2]->GLs[j - DOFs_surfaces1[surf1_index]];		//GLs da superfície 2
	...
	for (int i = 0; i < number_surfaces1; i++)
	{
		for (int j = 0; j < number_surfaces2; j++)
		{
			if (activate[i][j] == true && surf_pair[i][j] != NULL)
			{
				...
				//		//Critério numérico para indicar praticamente uma inversão do vetor normal de contato
